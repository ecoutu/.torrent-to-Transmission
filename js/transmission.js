	...
	req.onreadystatechange = function() {
		if (req.readyState == 4) {
			if (req.getResponseHeader("X-Transmission-Session-Id")) {
				localStorage.sessionId = req
						.getResponseHeader("X-Transmission-Session-Id");
...
function add_torrent_by_metainfo(link_url) {
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4) {
								...
								try {
									if (localStorage.getItem("lastStatus") == 200) {
										...
										if (rv["result"] == "success") {
											showNotification(
													"torrent started",
													rv["arguments"]["torrent-added"]["name"]);
										} else {
											showNotification(
										...
										var title = "unable to contact "
												+ localStorage
														.getItem("rpcURL");
										var text = "";
										switch (JSON.parse(localStorage
												.getItem("lastStatus"))) {
										case 0:
											text = "no response";
											break;
			...
			if (!good) {
				showNotification("use metainfo fail",
						"can't retrieve metainfo of torrent");
			}
		}
	}
	...
	rpc_request(json, function(req) {
		try {
			if (localStorage.getItem("lastStatus") == 200) {
				...
				var rv = JSON.parse(req.responseText);
				if (rv["result"] == "success") {
					showNotification("torrent started",
							rv["arguments"]["torrent-added"]["name"]);
				} else {
...
function update_torrents() {
	var json = JSON.stringify({
		"method" : "torrent-get",
		"arguments" : {
			"fields" : [ "id", "name", "status", "sizeWhenDone",
					"leftUntilDone", "percentDone", "rateDownload",
					"rateUpload", "peersConnected", "peersGettingFromUs",
					"peersSendingToUs", "recheckProgress", "queuePosition",
...
function update_session() {
	...
	rpc_request(json, function(req) {
		...
		try {
			localStorage.setItem("rpc_version",
					JSON.parse(req.response).arguments["rpc-version"]);
			if (localStorage.getItem("rpc_version") == 14) {
				TR_STATUS_STOPPED = 0;
